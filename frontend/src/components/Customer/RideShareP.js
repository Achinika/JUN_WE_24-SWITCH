...
function RideShareP() {
    ...
    const [rideShareP, setRideShareP] = useState([]);
    ...
    const fetchDetails = async () => {
        try {
            const response = await axios.get(global.APIUrl + '/rideShareP/all');
            const rideSharePWithId = response.data.map((rideShareP, index) => ({
                id: index + 1,
                ...rideShareP
            }));
            setRideShareP(rideSharePWithId);
        } catch (error) {
            console.error('Error fetching rideShareP details:', error);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.post(global.APIUrl + "/rideShareP/add", data)
            Swal.fire({
                title: "Success!",
                text: "Successfully.",
                icon: 'success',
            ...
            fetchDetails();
            setTimeout(() => {
                window.location.href = "/RideShareP";
            }, 1000);
        } catch (error) {
            console.log(error.message);
            Swal.fire({
                title: "Error!",
                text: "Failed RideShareP",
                icon: 'error',
                confirmButtonText: "OK"
            });
            setTimeout(() => {
    ...
    const handleEdit = (id) => {
        const data = rideShareP.find((rideShareP) => rideShareP.rid === id);
        setEmail(data.email);
        setEdtBtn(true);
        setEdrid(data.rid);
    ...
    const editFun = () => {
        if (edtBtn) {
            ...
            axios.put(global.APIUrl + "/rideShareP/update", data)
                .then((response) => {
                    Swal.fire({
                        title: "Success!",
                        text: "RideShareP Updated Successfully.",
                        icon: 'success',
                        confirmButtonText: "OK"
                    });
                    setTimeout(() => {
                ...
                .catch((error) => {
                    ...
                    Swal.fire({
                        title: "Error!",
                        text: "Failed to Update RideShareP",
                        icon: 'error',
                        confirmButtonText: "OK"
                    });
                    setTimeout(() => {
    ...
    const handleDelete = async (rid) => {
        try {
            await axios.delete(global.APIUrl + "/rideShareP/delete/" + rid);
            Swal.fire({
                title: "Success!",
                text: "RideShareP Deleted Successfully.",
                icon: 'success',
                confirmButtonText: "OK"
            });
            setTimeout(() => {
            ...
            Swal.fire({
                title: "Error!",
                text: "Failed to Delete RideShareP",
                icon: 'error',
                confirmButtonText: "OK"
            });
            setTimeout(() => {
    ...
    return (
        <div className={classes.root}>
            ...
            <div className={classes.header}>
                <Typography variant="h3" component="h1">Ride Sharing For Passenger</Typography>
            </div>
