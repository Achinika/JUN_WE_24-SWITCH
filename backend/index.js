...
// Define the route for switching to genaral user account
app.get('/auth/switchGenUser/:userId', async(req, res) => {
    const userId = req.params.userId;

    try {
        //find user by id
        const user = await UserModel.findById(userId).select('isGenUser');
